...
# tenta na ordem; usa o primeiro que responder (modelos mudam de nome com o tempo)
GEMINI_MODELS = ["gemini-2.5-flash", "gemini-2.0-flash", "gemini-flash-latest",
                 "gemini-2.5-flash-lite", "gemini-1.5-flash-latest", "gemini-1.5-flash"]
_GM = {"model": None}
...
def _gemini_json(prompt):
    ...
    # ordem: modelo que ja funcionou nesta rodada primeiro, depois o resto
    modelos = ([_GM["model"]] if _GM["model"] else []) + [m for m in GEMINI_MODELS if m != _GM["model"]]
    erro = None
    for model in modelos:
        ...
        try:
            ...
            if r.status_code != 200:
                erro = f"{model}: HTTP {r.status_code} {r.text[:200]}"
                continue
            j = r.json()
            txt = j["candidates"][0]["content"]["parts"][0]["text"]
            out = json.loads(txt)
            _GM["model"] = model  # cacheia o que funcionou
            return out
        except Exception as e:
            erro = f"{model}: {e}"
            continue
    raise RuntimeError(erro or "gemini falhou")
...
def coletar_fatos():
    ...
    return _interpretar(docs, _prompt_lote, "gemini_debug.json", "findings_visto.json", "fatos")


# ---- interpretador reutilizavel: manda os documentos ao Gemini em lotes,
#      classifica e marca o historico (visto_em). Serve tanto para CIA_ABERTA (acoes)
#      quanto para FNET (fundos). So muda o prompt e os arquivos de saida.
def _interpretar(docs, prompt_fn, dbg_nome, visto_nome, tag):
    com_texto = sum(1 for d in docs if d.get("texto"))
    saida, gemini_erro, lotes_ok = [], None, 0
    for i in range(0, len(docs), FATOS_LOTE):
        lote = docs[i:i + FATOS_LOTE]
        try:
            res = _gemini_json(prompt_fn(lote))
            lotes_ok += 1
            for it in (res.get("itens") or []):
                ...
                d = lote[idx]
                saida.append({
                    "empresa": d.get("empresa", ""), "cnpj": d.get("cnpj", ""),
                    "data": d.get("data", ""), "link": d.get("link", ""),
            ...
            if gemini_erro is None:
                gemini_erro = str(e)
            print(f"[{tag}] erro no lote:", e)
            traceback.print_exc()
        time.sleep(1)  # respeita rate limit do Gemini
    escrever(dbg_nome, {
        "docs_na_janela": len(docs), "docs_com_texto": com_texto,
        "lotes_enviados": (len(docs) + FATOS_LOTE - 1) // FATOS_LOTE, "lotes_ok": lotes_ok,
        "findings": len(saida), "gemini_modelo_usado": _GM["model"],
        "gemini_erro": gemini_erro, "tem_key": bool(GEMINI_KEY),
    })
...
# ============ FATOS/COMUNICADOS DE FUNDOS (FII/Fiagro/FI-Infra) via FNET ============
...
FNET_PAGINAS = 30       # teto de paginas (100 docs cada) por rodada
FNET_MAX = 90           # teto de docs mandados ao Gemini
FNET_CATEGORIAS = ("fato relevante", "comunicado ao mercado")  # filtra pelo texto da categoria

TIPOS_FUNDOS = ["Venda de Ativo", "Aquisição de Ativo", "Emissão/Follow-on",
                "Amortização/Provento Extraordinário", "Inadimplência/Default",
                "Reestruturação/RJ do Devedor", "Vacância/Locação",
                "Mudança de Gestão/Governança", "Recompra de Cotas",
                "Litígio/Regulatório", "Resultado/Distribuição", "Outro"]


def _prompt_lote_fundos(docs):
    itens = "\n\n".join(
        f"[{i}] Fundo: {d.get('empresa','')} (CNPJ {d.get('cnpj','')}) | Data: {d.get('data','')} | Categoria: {d.get('categoria','')}\n"
        f"Documento: {d.get('assunto','')}\nTexto: {d.get('texto','')[:3500]}"
        for i, d in enumerate(docs))
    tipos = ", ".join(f'"{t}"' for t in TIPOS_FUNDOS)
    return (
        "Voce e analista senior de special situations especializado em fundos listados na B3 "
        "(FIIs, Fiagro e fundos de infraestrutura). Abaixo estao fatos relevantes e comunicados ao mercado "
        "divulgados por esses fundos. Para CADA item, avalie se ha potencial de oportunidade de investimento "
        "(distressed, evento especial, assimetria de retorno, desconto sobre o patrimonio) e classifique.\n"
        "Retorne SOMENTE um JSON no formato {\"itens\":[{...}]} onde cada objeto tem:\n"
        "- idx: o numero entre colchetes do item\n"
        f"- tipo_evento: um de [{tipos}]\n"
        "- relevancia: inteiro de 1 a 5 (5 = oportunidade forte/assimetrica; 1 = burocratico/irrelevante)\n"
        "- oportunidade: true ou false\n"
        "- ticker: o codigo de negociacao do fundo na B3 se souber (ex HGLG11), senao \"\"\n"
        "- tese: 2 a 3 frases em portugues explicando o que aconteceu e por que e (ou nao) uma oportunidade\n\n"
        "Documentos:\n" + itens)
...
def _fnet_pagina(start, length, data_ini, data_fim):
    params = {"d": 1, "s": start, "l": length,
              "o[0][dataEntrega]": "desc",
              "dataInicial": data_ini, "dataFinal": data_fim}
    try:
        r = requests.get(FNET_URL, params=params, headers=_fnet_headers(), timeout=90)
    except Exception as e:
    ...
    try:
        ...
        return None, f"json invalido: {e} / {r.text[:180]}"
...
def coletar_fatos_fundos():
    ...
    cats, docs = {}, []
    total, erro, amostra, paginas = None, None, None, 0
    parar = False
    ...
    if erro and not docs:
        raise RuntimeError("FNET: " + erro)
    docs.sort(key=lambda d: d["data"] or "", reverse=True)
    if len(docs) > FNET_MAX:
        print(f"[fundos] {len(docs)} docs na janela; limitando a {FNET_MAX}")
        docs = docs[:FNET_MAX]
    for d in docs:
        d["texto"] = _fnet_texto(d["_id"]) if d.get("_id") else ""
    return _interpretar(docs, _prompt_lote_fundos, "gemini_fundos_debug.json", "findings_fundos_visto.json", "fundos")
...
FONTES = [("noticias", "noticias.json", coletar_noticias),
          ...
          ("fatos_fundos", "findings_fundos.json", coletar_fatos_fundos)]
